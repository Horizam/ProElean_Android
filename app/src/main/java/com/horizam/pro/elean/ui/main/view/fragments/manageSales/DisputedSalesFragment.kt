...
class DisputedSalesFragment : Fragment(), OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {

    private lateinit var binding: FragmentOrdersGenericBinding
    private lateinit var adapter: DisputedSalesAdapter
    ...
    private fun exeApi() {
        viewModel.getSellerOrdersCall(BuyerOrders.Disputed)
    }

    private fun initViews() {
        adapter = DisputedSalesAdapter(this)
    ...
    override fun <T> onItemClick(item: T) {
        if (item is Order) {
            Intent(requireContext(), OrderDetailsActivity::class.java).also {
                ...
                it.putExtra(Constants.ORDER_USER_ACTION, SellerOrders.Disputed)
