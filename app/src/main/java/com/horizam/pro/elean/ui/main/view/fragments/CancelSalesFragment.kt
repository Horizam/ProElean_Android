...
import com.horizam.pro.elean.ui.main.adapter.CancelOrdersAdapter
import com.horizam.pro.elean.ui.main.adapter.CancelSalesAdapter
...
class CancelSalesFragment : Fragment(), OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {

    private lateinit var binding: FragmentOrdersGenericBinding
    private lateinit var adapter: CancelSalesAdapter
    ...
    private fun exeApi() {
        viewModel.getSellerOrdersCall(BuyerOrders.Cancel)
    }

    private fun initViews() {
        adapter = CancelSalesAdapter(this)
...
//    }
    ...
    private fun setAdapterLoadState(adapter: CancelSalesAdapter) {
    ...
    override fun <T> onItemClick(item: T) {
        if (item is Order) {
            Intent(requireContext(), OrderDetailsActivity::class.java).also {
                ...
                it.putExtra(Constants.ORDER_USER_ACTION, SellerOrders.Cancel)
