...
class CompletedSalesFragment : Fragment(), OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {

    private lateinit var binding: FragmentOrdersGenericBinding
    private lateinit var adapter: CompletedSalesAdapter
    ...
    private fun exeApi() {
        viewModel.getSellerOrdersCall(BuyerOrders.Completed)
    }

    private fun initViews() {
        adapter = CompletedSalesAdapter(this)
    ...
    override fun <T> onItemClick(item: T) {
        if (item is Order) {
            Intent(requireContext(), OrderDetailsActivity::class.java).also {
                ...
                it.putExtra(Constants.ORDER_USER_ACTION, SellerOrders.Completed)
